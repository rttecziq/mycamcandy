...
export class InvoiceComponent implements AfterViewInit{
    ...
    ngAfterViewInit(){
        ...
        let details = {subscription_id : this.subscription_id};

        this.invoiceDetailsFn('subscription/invoice', details);
    }
    ...
    // Apply coupon for subscription

    applyCouponForSubscription(coupon_code) {

        let details = {subscription_id : this.subscription_id , coupon_code : coupon_code};

        this.requestService.postMethod('apply/coupon/subscription', details)
            .subscribe(
        ...
        if (payment_type) {
            ...
            if (parseInt(total_remaining_amount) > 0) {
                ...
                if (payment_type == 1) {
                    ...
                    this.paypalPayment(this.requestService.adminUrl+"paypal/", {subscription_id : this.subscription_id, user_id : user_id, coupon_code : this.coupon_code});

                } else {

                    this.cardPayment("stripe_payment", {subscription_id : this.subscription_id, coupon_code : this.coupon_code});
                }

            } else {

                this.zeroPlan("pay_now", {subscription_id : this.subscription_id, payment_id : 'COUPON-DISCOUNT', coupon_code : this.coupon_code});

            }

        } else {

            this.zeroPlan("pay_now", {subscription_id : this.subscription_id, payment_id : 'FREE-PLAN'});
            ...
            .subscribe(
                (data : any) => {
                    ...
                    if (data.success == true) {
                        ...
                        this.router.navigate(['/payment-success'], {queryParams : {subscription_id:this.subscription_id}});
    ...
    paypalPayment(url, object) {
        ...
        if(object.coupon_code == '' || object.coupon_code == null || object.coupon_code == "undefined") {
            ...
            window.location.href = url+object.subscription_id+"/"+object.user_id;

        } else {

            window.location.href = url+object.subscription_id+"/"+object.user_id+"/"+object.coupon_code;
                ...
                (data : any) => {
                    ...
                    if (data.success == true) {
                        ...
                        if (data.error_code == 901) {
                            ...
                            this.router.navigate(['/add-card'], {queryParams : {subscription_id : this.subscription_id}});
